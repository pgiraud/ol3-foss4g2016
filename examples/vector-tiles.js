...
function styleFunction(feature, resolution) {
  ...
  if (layer == 'landuse' && cls == 'park') {
    ...
    stroke.setWidth(constants['@motorway_link_casing_width'].base);
    styles[length++] = line;
  } else if (layer == 'tunnel' && cls == 'service') {
    stroke.setColor(constants['@street_casing']);
    stroke.setWidth(constants['@service_casing_width'].base);
    styles[length++] = line;
  } else if (layer == 'tunnel' &&
      (cls == 'street' || cls == 'street_limited')) {
    stroke.setColor(constants['@street_casing']);
    stroke.setWidth(constants['@street_casing_width'].base);
    styles[length++] = line;
  } else if (layer == 'tunnel' && cls == 'main' && zoom >= 7) {
    stroke.setColor(constants['@motorway_casing']);
    stroke.setWidth(constants['@main_casing_width'].base);
    styles[length++] = line;
  } else if (layer == 'tunnel' && cls == 'motorway') {
    stroke.setColor(constants['@motorway_casing']);
    stroke.setWidth(constants['@motorway_casing_width'].base);
    styles[length++] = line;
  } else if (layer == 'tunnel' && cls == 'path') {
    stroke.setColor(constants['@path']);
    stroke.setWidth(constants['@path_width'].base);
    styles[length++] = line;
  } else if (layer == 'tunnel' && cls == 'major_rail') {
    stroke.setColor(constants['@rail']);
    stroke.setWidth(constants['@rail_width'].base);
    styles[length++] = line;
    ...
    stroke.setWidth(constants['@admin_level_3_width'].base);
    styles[length++] = line;
  } else if (layer == 'admin' && adminLevel == 2 &&
      disputed === 0 && maritime === 0) {
    stroke.setColor(constants['@admin']);
    stroke.setWidth(constants['@admin_level_2_width'].base);
    styles[length++] = line;
  } else if (layer == 'admin' && adminLevel == 2 &&
